...
export default function AddCase () {
    ...
    const declare_emergency = (e: FormEvent)=>{
        ...
        let nhs = (document.getElementById('nhs') as HTMLSelectElement).value;
        ...
        axios.post('/api/add-case', form, {withCredentials:true}).then(response => {
            let id = response.data.id;
            const pat:FormData = new FormData();
            pat.append('dob', dob);
            pat.append('name', pname);
            pat.append('injury', des);
            pat.append('address', address);
            axios.post(`http://localhost:8080/addFromKwik/${nhs}`, pat).then(response =>{
                axios.get(`http://localhost:8888/api/assign/${id}`).then((res) =>{
                   if (res.data['msg'] === 'success') {
                       axios.get(`http://localhost:8080/updateFromKwik/${nhs}/${res.data['code']}`).then((res) =>{
                           if (res.data['msg'] === 'success') {
                               navigate(`/track/${id}`);
                           }
                       })
                ...
                }).catch(error =>{alert('error adding hospital')})
                                ...
                                <div style={{
                                    ...
                                    justifyContent: 'center'
                                        ...
                                        <span style={{color: '#777', fontSize: 19, marginRight: 10}}
                                              className="material-symbols-outlined">
                                             description
                                        </span> <p style={{color: '#777', fontSize: 14}}>Patient NHS Number</p>
                                </div>
                                <input placeholder='0000011111' className='noner' id="nhs"/>
                                <br/>
